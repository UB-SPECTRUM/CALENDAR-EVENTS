<?php
    ...
    class EventCategories extends DatabaseConnector {
        ...
        public static function getAll(){
            ...
            $fetchCategoriesQuery = "SELECT CATEGORY_ID, NAME, ICON, DESCRIPTION FROM tbl_categories";
        ...
        public static function getCategoriesForEvent($eventId) {
            ...
            $fetchCategoriesQuery =$conn->prepare("SELECT tbl_categories.CATEGORY_ID AS CATEGORY_ID, tbl_categories.NAME AS NAME, tbl_categories.ICON AS ICON, tbl_categories.DESCRIPTION AS DESCRIPTION FROM tbl_categories INNER JOIN tbl_event_categories ON tbl_categories.CATEGORY_ID = tbl_event_categories.CATEGORY_ID WHERE tbl_event_categories.EVENT_ID = ?");
        ...
        public static function getRecurringCategoriesForEvent($eventId){
            ...
            $fetchCategoriesQuery =$conn->prepare("SELECT tbl_categories.CATEGORY_ID AS CATEGORY_ID, tbl_categories.NAME AS NAME, tbl_categories.ICON AS ICON, tbl_categories.DESCRIPTION AS DESCRIPTION FROM tbl_categories INNER JOIN tbl_recur_event_categories ON tbl_categories.CATEGORY_ID = tbl_recur_event_categories.CATEGORY_ID WHERE tbl_recur_event_categories.EVENT_ID = ?");
            $fetchCategoriesQuery->bind_param("i", $eventId);
            $fetchCategoriesQuery->execute();
            $result = $fetchCategoriesQuery->get_result();
            if($result != NULL){
