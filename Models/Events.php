<?php
    ...
    class Events extends DatabaseConnector {
        ...
        public static function getEventRecurringFlyer($eventId){
            $temparray = array();
            $conn = self::getDB();
            $eventId = $conn->real_escape_string($eventId);
            $fetchFlyerQuery = "SELECT RECURING_EVENT_ID, NAME, ADDITIONAL_FILE, ADDITIONAL_FILE_SIZE, ADDITIONAL_FILE_TYPE FROM tbl_recur_events WHERE ID=$eventId;";
            $result = mysqli_query($conn,$fetchFlyerQuery);
        ...
        public static function getEventInfo($eventId){
            $fetchedEvent = array("ID"=>'', "NAME"=>'', "VENUE"=>'',"DATE"=> '', "START_TIME" => '', "END_TIME"=>'',"CATEGORY"=>'',"DESCRIPTION"=>'',"LINK"=>'',"PHONE"=>'',"EMAIL"=>'',"COST"=>'',"UB_CAMPUS_LOCATION"=>'', "APPROVAL_STATUS"=>'', "ADDITIONAL_FILE_TYPE" => '',"ADDITIONAL_FILE_SIZE" => '', "ADDITIONAL_FILE" =>'' );
            $conn = self::getDB();
            $eventId = $conn->real_escape_string($eventId);
